...
/**
 * Implementation of OrientDB's {@code OCompression} interface to provide encryption of stored records.
 *
 * @since 3.0
 */
public class EncryptedRecordObfuscator
    implements OCompression
{
  public static final String NAME = "encryptedRecordObfuscator";
  ...
  @Inject
  public EncryptedRecordObfuscator(final CryptoHelper cryptoHelper,
                                   final @Named(CPREFIX + ".password:-changeme}") String password,
                                   final @Named(CPREFIX + ".salt:-changeme}") String salt,
                                   final @Named(CPREFIX + ".iv:-0123456789ABCDEF}") String iv) throws Exception {
