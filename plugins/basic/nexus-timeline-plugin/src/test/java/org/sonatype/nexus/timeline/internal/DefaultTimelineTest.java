    ...
    extends TestSupport
{
  ...
  public void prepare() throws Exception {
    ...
    final Module testModule = new AbstractModule()
    {
      ...
      protected void configure() {
        ...
        bind(DatabaseManager.class).to(DatabaseManagerImpl.class).asEagerSingleton();
  ...
  @Test
  public void lotOfEvents() throws Exception {
    for (int j = 0; j < 100000; j++) {
      final Entry[] entries = new Entry[1000];
      for (int i = 0; i < 1000; i++) {
        final Map<String, String> data = Maps.newHashMap();
        data.put("a", String.valueOf(System.nanoTime()));
        data.put("b", "something almost constant " + new DateTime());
        entries[i] = new EntryRecord(System.currentTimeMillis(), "alpha", "beta", data);
      }
      defaultNexusTimeline.add(entries);
    }
